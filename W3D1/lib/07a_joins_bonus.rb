...
def expensive_tastes
  ...
  execute(<<-SQL)
  select
    styles.style, (sum(albums_count.price)/sum(albums_count.count))
  from
    styles
  join (
  select
  ...
  group by albums.asin) as albums_count on styles.album = albums_count.asin
  group by styles.style
  order by (sum(albums_count.price)/sum(albums_count.count)) DESC
  limit 5
